...
print "Serial COM port names that this virtGPIO.py tests for", GPIO.portlist
print "Serial baudrates that this virtGPIO.py tests for", GPIO.baudlist

print "Serial baudrate", GPIO.Serial.baudrate
print "Serial port", GPIO.Serial.port
...
print "10 LED toggles high speed, no flow control, no problem, (not many serial characters to transfer)"
for k in range(10):
    GPIO.output(led, k&1) # toggle led.
    # This output command has no return from arduino
    # Doing this 10 times is not a problem. What is serial buffer capacity. In hundreds ?????
print "Done"
time.sleep(3)

print "5000 LED toggles high speed, with sync() stream control"
for k in range(5000):
    GPIO.output(led, k&1) # toggle led.
    # Doing 5000 of these at high speed is likely to overrun the serial stream from rPi to Arduino.
    GPIO.sync()
    # so we send a "ping" with a return, just to keep both ends in sync.
    # SEE QUADENCODER EXAMPLE FOR GOOD USE OF SYNC()
print "Done"
time.sleep(4)
