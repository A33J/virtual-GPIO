 ...
 */
void Stepper::init(int number_of_steps, int motor_pin_1, int motor_pin_2)
{
  ...
  this->_last_step_time = 0;    // time stamp in ms of the last step taken
...
/*
 *   init for four-pin version
 *   Sets which wires should control the motor.
 */

void Stepper::init(int number_of_steps, int motor_pin_1, int motor_pin_2, int motor_pin_3, int motor_pin_4)
{
  this->_step_number = 0;      // which step the motor is on
  this->_speed = 0;        // the motor speed, in revolutions per minute
  this->_direction = 0;      // motor direction
...
{
  this->_step_delay = 60L * 1000L / this->_number_of_steps / whatSpeed;
}
...
{
  this->_steps_left = abs(steps_to_move);  // how many steps to take

  // determine direction based on whether steps_to_mode is + or -:
  if (steps_to_move > 0) {this->_direction = 1;}
  if (steps_to_move < 0) {this->_direction = 0;}
}

void Stepper::run()
{
  if(this->_steps_left == 0)
    return;
  ...
  _stepMotor(this->_step_number % 4);
 ...
 */
void Stepper::_stepMotor(int thisStep)
{
  if (this->_pin_count == 2) {
    switch (thisStep) {
      case 0: /* 01 */
